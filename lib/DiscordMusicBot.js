...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    this.manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(
          `Node: ${node.options.identifier} | Lavalink đã kết nối.`
        )
      )
      .on("nodeReconnect", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Lavalink đang kết nối lại.`
        )
      )
      .on("nodeDestroy", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Lavalink đã bị hủy.`
        )
      )
      .on("nodeDisconnect", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Lavalink đã ngắt kết nối.`
        )
      )
      .on("nodeError", (node, err) => {
        this.warn(
          `Node: ${node.options.identifier} | Lavalink có lỗi: ${err.message}.`
        );
      })
          ...
          if (autoQueue) {
            ...
            try {
              if (!player.playing && !twentyFourSeven) {
                setTimeout(async () => {
                  if (!player.playing && player.state !== "DISCONNECTED") {
                    let disconnectedEmbed = new MessageEmbed()
                      ...
                      .setDescription(
                        `Danh sách phát đã bị ngắt kết nối do không có hoạt động.`
                      );
                ...
                client.warn(
                  `Có điều không mong muốn đã xảy ra ${player.options.guild}`
                );
              }
              player.setNowplayingMessage(client, null);
            } catch (err) {
